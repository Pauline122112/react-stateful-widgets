...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return programmersList[activeProgrammer -1].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: `${activeProgrammer === null ? 'royalblue' : 'gold'}`// 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setActiveProgrammer(programmersList.id)}}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          activeProgrammer ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳`
